...
class Portfolio():
    ...
    def treat_csv_files(self, path_to_folder: str):
        ...
        if len(asset_csv_names) != len(path_csvs_availables): exit("Error, the csv files of the desired assets are not available in the specified path (path to folder csv files from webscraping cannot be reached)")
        csvs_treated = {}
        type_asset_index = 0
